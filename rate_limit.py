...
class RateLimitMiddleware(BaseHTTPMiddleware):
    """Aplica un RateLimiter distinto por ruta (POST únicamente), identificando
    al llamante por IP. Rutas sin límite configurado pasan sin tocar."""

    def __init__(self, app, limiters: dict[str, RateLimiter]) -> None:
        super().__init__(app)
        self._limiters = limiters

    async def dispatch(self, request: Request, call_next):
        limiter = self._limiters.get(request.url.path) if request.method == "POST" else None
